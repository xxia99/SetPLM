...
class TransformerSeq2SeqDecoder(nn.Module):
    def __init__(self, embed, pos_embed,
                 ...
                 fix_kp_num_len=False, max_kp_len=6, max_kp_num=20, vocab_size=None, decoder=None, plm=None, tk=None):
        """
        ...
        if copy_attn:
            ...
            self.p_gen_linear = nn.Linear(768, 1)
        ...
        if self.fix_kp_num_len:
            ...
            self.control_code = nn.Embedding(max_kp_num // 2, self.embed.embedding_dim)
            self.control_code.weight.data.uniform_(-0.1, 0.1)
            # self.control_code_4 = nn.Embedding(max_kp_num // 4, self.embed.embedding_dim)
            # self.control_code_4.weight.data.uniform_(-0.1, 0.1)
            self.self_attn_mask = self._get_self_attn_mask(max_kp_num, max_kp_len)
            self.kws_embedding = nn.Embedding(vocab_size, self.embed.embedding_dim)
        if plm:
            self.plm = plm
            self.decoder = decoder
            if self.plm and self.decoder:
                # for param in self.plm.encoder.parameters():
                #     param.requires_grad = False
                # for param in self.plm.parameters():

                    # param.requires_grad = False
                self.tk = tk
                self.t5_embed = nn.Linear(512, 768)
                self.layer = TransformerSeq2SeqDecoderLayer(d_model, n_head, dim_ff, dropout, 0,
                                                            fix_kp_num_len, max_kp_num)
                self.t5tox = nn.Linear(768, 512)
                self.t5_output_layer = nn.Linear(768, self.vocab_size, bias=False)
    ...
    def forward_seg(self, state, kws):
        # encoder_output = state.encoder_output
        # batch_size = encoder_output.size(0)
        # device = encoder_output.device
        # control_kws = torch.tensor(kws).long().to(device)
        # # control_idx = torch.arange(0, self.max_kp_num).long().to(device).reshape(1, -1)
        ...
        # # dim = control_kws.size(1)
        # # kws_embed_layer = nn.Embedding(dim, self.embed.embedding_dim)
        # kws_embed = self.control_code(control_kws)
        # return control_embed, kws_embed
        encoder_output = state.encoder_output
        batch_size = encoder_output.size(0)
        device = encoder_output.device
        ...
        # control_idx = torch.arange(0, self.max_kp_num).long().to(device).reshape(1, -1).repeat(batch_size, 1)
        # control_embed = self.control_code(control_idx)
        # kws_embed = self.kws_embedding(kws.to(device))


        ###### new kws embed ######
        control_idx = torch.arange(0, self.max_kp_num // 2).long().to(device).reshape(1, -1).repeat(batch_size, 1)
        control_embed = self.control_code(control_idx)
        kws_10 = kws[:, :self.max_kp_num // 2].to(device)
        kws_embed = self.kws_embedding(kws_10)
        kws_embed = torch.concat((kws_embed, control_embed), dim=1).to(device)

        # control_idx = torch.arange(0, self.max_kp_num // 4).long().to(device).reshape(1, -1).repeat(batch_size, 1)
        # control_embed = self.control_code_4(control_idx)
        # kws_10 = kws[:, :self.max_kp_num // 2].to(device)
        # kws_5 = kws[:, :self.max_kp_num // 4].to(device)
        # kws_15 = kws[:, :(self.max_kp_num - self.max_kp_num // 4)].to(device)
        # kws_20 = kws[:, :self.max_kp_num].to(device)
        # kws_embed_10 = self.kws_embedding(kws_10)
        # kws_embed_5 = self.kws_embedding(kws_5)
        # kws_embed_15 = self.kws_embedding(kws_15)
        # kws_embed = self.kws_embedding(kws_20)
        # kws_embed = torch.concat((kws_embed_10, kws_embed_5, control_embed), dim=1).to(device)
        # kws_embed = torch.concat((kws_embed_15, control_embed), dim=1).to(device)

        return control_embed, kws_embed

    def forward(self, tokens, state, src_oov, max_num_oov, control_embed=None, src=None, src_mask=None):
        """
        ...
        if self.fix_kp_num_len:
            ...
            # print("decode_vocab: ", self.vocab_size)
            ...
            if self.plm and self.decoder:
                ...
                control_embed = control_embed.reshape(batch_size, max_kp_num, 1, -1)
                # embed = self.input_fc(word_embed) + control_embed
                embed = self.input_fc(word_embed) + position_embed + control_embed
                embed = F.dropout(embed, p=self.dropout, training=self.training)
                embed = embed.to(device).reshape(batch_size, max_kp_num * kp_len, -1)
                if kp_len > 1:  # training
                    self_attn_mask = self.self_attn_mask
                else:
                    self_attn_mask = self.self_attn_mask.to(device).reshape(max_kp_num, self.max_kp_len, max_kp_num,
                                                                 self.max_kp_len) \
                        [:, decode_length, :, :decode_length + 1] \
                        .to(device).reshape(max_kp_num, max_kp_num * (decode_length + 1))

                x, attn_dist = self.layer(x=embed,
                                          encoder_output=encoder_output,
                                          encoder_mask=encoder_mask,
                                          self_attn_mask=self_attn_mask.to(device),
                                          state=state
                                          )
                decoder_inputs_embeds = self.t5_embed(embed)
                inputs_embeds = self.t5_embed(encoder_output)
                encoder_output = (self.t5_embed(encoder_output),)
                outputs = self.plm(
                    input_ids=src,
                    attention_mask=src_mask,
                    # encoder_outputs=encoder_output,  # inputs_embeds
                    # inputs_embeds=inputs_embeds,
                    decoder_inputs_embeds=decoder_inputs_embeds)
                # logits = outputs.logits
                # vocab_dist = F.softmax(logits, -1)

                x = outputs.last_hidden_state
                # x = self.t5tox(x)
                # x = self.layer_norm(x)  # batch, tgt_len, dim
                # x = self.output_fc(x)
                vocab_dist = F.softmax(self.t5_output_layer(x), -1)


                attn_dist = outputs.cross_attentions[0]
