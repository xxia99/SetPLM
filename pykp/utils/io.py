...
class KeyphraseDataset(torch.utils.data.Dataset):
    def __init__(self, examples, word2idx, idx2word, device, load_train=True,
                 fix_kp_num_len=False, max_kp_len=6, max_kp_num=20, seperate_pre_ab=False, plm=None):
        keys = ['src', 'src_oov', 'oov_dict', 'oov_list', 'src_str', 'trg_str', 'trg', 'trg_copy','kws', 'kws_str']
    ...
    def collate_fn_common(self, batches, trg=None, trg_oov=None):
        # print("!!!!!!Hello!!!!!!")
        # print("io_vocab: ", len(self.word2idx.items()))
        if self.plm:
            kws = [b['kws'][:512] for b in batches]
            kws_str = [b['kws_str'][:512] for b in batches]
            kws = [kw + [0] * (self.max_kp_num - len(kw)) for kw in kws]
            src = [b['src'][:512] for b in batches]
            # extended src (oov words are replaced with temporary idx, e.g. 50000, 50001 etc.)
            src_oov = [b['src_oov'][:512] for b in batches]

            oov_lists = [b['oov_list'][:512] for b in batches]

            # b['src_str'] is a word_list for source text
            # b['trg_str'] is a list of word list
            src_str = [b['src_str'][:512] for b in batches]
            trg_str = [b['trg_str'][:512] for b in batches]
        else:
            # source with oov words replaced by <unk>
            kws = [b['kws'] for b in batches]
            kws_str = [b['kws_str'] for b in batches]
            kws = [kw[:self.max_kp_num] for kw in kws]
            kws = [kw + list(range(self.max_kp_num - len(kw))) for kw in kws]
        ...
        seq_pairs = sorted(zip(src_lens, src, src_oov, oov_lists, src_str, trg_str, original_indices, kws, kws_str),
                           key=lambda p: p[0], reverse=True)
        _, src, src_oov, oov_lists, src_str, trg_str, original_indices, kws, kws_str = zip(
            *seq_pairs)
        ...
        return src, src_lens, src_mask, src_oov, oov_lists, src_str, \
               trg_str, trg, trg_oov, trg_lens, trg_mask, original_indices, kws_str, kws
        ...
        if self.load_train:
            if self.seperate_pre_ab:
                ...
                for b in batches:
                    ...
                    # print("io vocab: ", len(self.word2idx.items()))
                    # print("io peos id: ", self.word2idx['<peos>'])
                    # print("src_str", b['src_str'])
                    # print("targets_str", b['trg_str'])
                    ...
                    kw = [[k] for k in b["kws"]]
                    absent_targets += kw
                    absent_targets_oov += kw
                    absent_targets = absent_targets[:self.max_kp_num // 2]
                    absent_targets_oov = absent_targets_oov[:self.max_kp_num // 2]

                    present_targets += kw
                    present_targets_oov += kw
                    present_targets = present_targets[:self.max_kp_num // 2]
                    present_targets_oov = present_targets_oov[:self.max_kp_num // 2]
...
def build_interactive_predict_dataset(tokenized_src, kws, opt, mode="one2many", include_original=True):
    ...
    tokenized_src_trg_pairs = list(zip(tokenized_src, tokenized_trg, kws))
    #print(tokenized_src_trg_pairs)
...
def build_dataset(src_trgs_pairs, opt, mode='one2one', include_original=True):
    ...
    for idx, (source, targets, keyword) in enumerate(src_trgs_pairs):
        ...
        kws = [word2idx[w] if w in word2idx and word2idx[w] < opt.vocab_size else word2idx[UNK_WORD] for w in keyword]
        ...
        if mode == 'one2many' and len(examples) > 0:
            ...
            for key in keys:
                if key.startswith('src') or key.startswith('oov') or key.startswith('title') or key.startswith('kws'):
