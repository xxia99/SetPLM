    ...
    for i, pred in enumerate(prediction):
        ...
        if _pred < vocab_size:
            if _pred == unk_idx and replace_unk:
                ...
                # print('attn_dist: ', attn_dist[i].size())
                # attn_mask = torch.zeros(attn_dist[i].size())
                _, max_attn_idx = attn_dist[i].topk(2, dim=0)
                # print('src_len: ', len(src_word_list))
                # print('max_attn_0', max_attn_idx[0])
                # print('max_attn_1', max_attn_idx[1])
