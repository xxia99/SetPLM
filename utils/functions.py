...
def read_tokenized_src_file(path, remove_title_eos=True, prompt=''):
    """
...
def read_src_and_trg_files(src_file, trg_file, kws_file, is_train, remove_title_eos=True, prompt=""):
    print("!!!!!!Hello!!!!!!")
    tokenized_train_src = []
    tokenized_train_trg = []
    tokenized_train_kws = []
    filtered_cnt = 0
    with open(kws_file) as f_kws:
        lines = json.load(f_kws)
        kws_lines = lines["keywords"]
    for line_idx, (src_line, trg_line, kws_line) in enumerate(zip(open(src_file, 'r'), open(trg_file, 'r'), kws_lines)):
        if len(trg_line) > 1:
            # print(src_line)
            src_line = prompt + src_line
        # process source line
        ...
        tokenized_train_kws.append(kws_line)
    ...
    tokenized_train_pairs = list(zip(tokenized_train_src, tokenized_train_trg, tokenized_train_kws))
