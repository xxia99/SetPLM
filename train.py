...
from utils.data_loader_old import load_data_and_vocab
...
def init_optimizer(model, opt):
    ...
    optimizer = AdamW(params=filter(lambda p: p.requires_grad, model.parameters()),
                      lr=opt.learning_rate, betas=(0.9, 0.999), eps=1e-09)
    # optimizer = Adafactor(params=filter(lambda p: p.requires_grad, model.parameters()),
