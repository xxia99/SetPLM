...
from utils.data_loader_old import load_vocab, build_data_loader
...
def main(opt):
    ...
    # print("vocab_length: ", len(vocab["word2idx"]))
    # opt.vocab_size = len(vocab['word2idx'])
    src_file = opt.src_file
    kws_file = opt.kws_file
    tokenized_src = read_tokenized_src_file(src_file, remove_title_eos=opt.remove_title_eos, prompt=opt.prompt)
    #print("!!!!!!!!!!!!!!", tokenized_src[0])
    with open (kws_file)as f_kw:
        kws_dict = json.load(f_kw)
        kws = kws_dict['keywords']
    #print("!!!!!!!!!!!!!!", kws[0])
    if opt.one2many:
        mode = 'one2many'
    else:
    ...
    test_data = build_interactive_predict_dataset(tokenized_src, kws, opt, mode=mode, include_original=True)
